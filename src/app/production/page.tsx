...
export default function Production() {
  const [produtoCorreto, setProdutoCorreto] = useState<string>('');
  const [produtos, setProdutos] = useState<string[]>(['Produto A', 'Produto B']);

  const registraProdutoCorreto = (produto: string) => {
    setProdutoCorreto(produto);
  }

  const reconfigurarRodada = () => {
    setProdutoCorreto('');
  }
    ...
    <>
      ...
      >
        <div className="flex justify-center items-center">
          <h1 className="ml-3 text-3xl font-bold">Produção</h1>
        </div>
      </div>
      ...
      {!produtoCorreto ? (
        ...
        <div className="flex flex-col items-center">
          <p className="text-sm">Produto selecionado: <span className="font-semibold text-md">( {produtoCorreto} )</span></p>
          <button 
            onClick={reconfigurarRodada}
      ...
      <div className="mb-32 grid text-center lg:mb-0 lg:w-full lg:max-w-5xl lg:grid-cols-2 lg:text-left">
        {produtos.map((produto, index) => (
          <button
            ...
            onClick={() => registraProdutoCorreto(produto)}
          >
            <div className="flex flex-col items-center">
